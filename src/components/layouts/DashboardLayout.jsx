    ...
    <div className="min-h-screen bg-k8s-bg flex flex-col font-sans">
      ...
      <header className="bg-k8s-blue text-white shadow-md z-30 sticky top-0 flex items-center justify-between px-4 py-3 h-16 relative">
        ...
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-10">
          ...
          >
            <svg
              className="w-8 h-8 mr-2.5 text-white"
              viewBox="0 0 256 250"
              fill="currentColor"
            >
            ...
            <span className="font-bold text-lg tracking-wider hidden sm:inline">
              KUBERNETES
            </span>
          </Link>
        </div>
            ...
            <nav className="px-3 space-y-1">
              {menuItems.map((item) => {
                ...
                return (
                  ...
                  >
                    <Icon
                      className={`w-4 h-4 mr-3 ${isActive ? "text-k8s-blue" : "text-gray-400"}`}
                    />
                    {item.name}
                  </Link>
                );
